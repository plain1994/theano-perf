...
if len(sys.argv) == 1:
    ...
    if sys.argv[1] == 'gem':
        ...
        if (status == 0):
            print ("CPU MHz: %s" %(output.split(':')[1].strip()))


        status = os.system('g++ dgemm_with_timing.c -o a.out -lmkl_rt')
        if (status == 0):
            print("Compile gem test code done")
        ...
        if (status == 0):
            #print output
            print("Disk write test done, write speed: %s" %(output.split('\n')[2].split(',')[2].strip()))
            (status2, output) = commands.getstatusoutput('dd if=ddfile of=/dev/null bs=8k count=250000')
            if (status2 == 0):
                print("Disk read test done, read speed: %s" %(output.split('\n')[2].split(',')[2].strip()))
